...
Ambos casos requieren convertir texto a números, y una variable de texto a ordinal: 
...
Esta es la data original de IDH:
...
## <font color="red">Estrategia de Partición</font>
...
Exploremos las características estadísticas de cada cluster:
...
pamPlot + geom_text_repel(size=4, aes(label=PAMlabels), max.overlaps = 50,min.segment.length = unit(0, 'lines'))
```
...
agnesPlot + geom_text_repel(size=4, aes(label=AGNESlabels),min.segment.length = unit(0, 'lines'))
...
dianaPlot + geom_text_repel(size=4,aes(label=DIANAlabels), max.overlaps = 50,min.segment.length = unit(0, 'lines'))
